...
- Phase 4 (Calendar): complete (PRs #16–#17). Backend: `/api/calendar` router + controller (calendars CRUD, events CRUD, range queries with recurring-event expansion via `services/recurrence.ts` `occurrencesBetween`, `includeTasks` task overlay, search), `.ics` export/import (`services/ics.ts`, UID-deduped), migration 0003 applied to Neon, smoke test (`scripts/calendar-smoke.mjs`, 31 checks). Mobile: Calendar tab with Month/Week/Agenda views + to-do overlay, event detail, calendars management with device-calendar sync (`lib/device-calendar.ts`, one-way via expo-calendar → `.ics` import) and file import, search, NL quick entry (`lib/event-parser.ts`), `lib/calendar-api.ts`, `hooks/use-calendar.ts`, `lib/schemas/calendar.schemas.ts`. Deferred: two-way device sync, event reminder notifications, provider OAuth calendar sync.
- Phases 5–7 (Notes, Finance, polish): not started. Those module tabs render `ModulePlaceholder`; auth + mail + tasks + calendar routers are mounted in `backend/src/index.ts`, the rest are stub comments.
...
- **Neon migrations are per-environment.** Each v0 chat may connect a fresh Neon database. Always run `npm run db:migrate` in `backend/` before live-testing in a new environment. Migrations 0000–0003 and the live calendar smoke test (`backend/scripts/calendar-smoke.mjs`, 31/31) were verified on 2026-07-16 against the Neon DB connected to the current chat.
...
- Calendar deferrals: device-calendar sync is one-way (device → app; no write-back), event reminder notifications don't fire yet (`reminder_minutes` stored only), no provider OAuth calendar sync (device + `.ics` import only).
