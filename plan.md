...
|---|---|---|
| iOS (device support) | **iOS 18+** | Product decision: support previous + current major versions only (iOS 18 and iOS 26 — Apple skipped 19–25). Above RN 0.86's technical floor of 15.1, so fully compatible |
| Android (device support) | **Android 15+ (API 35)** | Product decision: previous + current major versions only (Android 15 and 16). Above RN 0.86's technical floor of API 24, so fully compatible |
...
- Pin these explicitly via `expo-build-properties` in `app.json` (`ios.deploymentTarget: "18.0"`, `android.minSdkVersion: 35`) so builds fail loudly instead of drifting.
- **OS support policy:** each year when new OS versions ship, raise the floor so the app always supports the previous + current major versions (e.g., when iOS 27 ships, the floor moves to iOS 26). Raising the minimum also lets you adopt new OS APIs (widgets, App Intents, Live Activities) without legacy fallback code.
...
- **Testing:** Jest + RN Testing Library (unit), Supertest (API), Maestro (E2E), device matrix **iOS 18 + iOS 26 / Android 15 + Android 16** — exactly the two supported major versions per platform, keeping the test matrix small and focused
...
7. OS floors pinned via `expo-build-properties` (`ios.deploymentTarget: "18.0"`, `android.minSdkVersion: 35`, `android.targetSdkVersion: 36`) so platform requirements never drift silently.
8. OS support policy: previous + current major OS versions only (today: iOS 18 & 26, Android 15 & 16) — floor is raised each year when new OS versions ship.
